...
ui_confirm() {
    ...
    while true; do
        ...
        case "$normalized" in
            ...
            *) printf "Please answer y, n, or exit (got '%s').\n" "$reply" >&2 ;;
        esac
    done
}
...
ui_secret() {
    ...
    if have_cmd gum; then
        if ! value="$(gum input --prompt "$prompt " --password)"; then
            printf 'exit\n'
            return 0
        fi
        printf '%s\n' "$value"
...
ui_select() {
    ...
    if have_cmd gum; then
        gum choose "${options[@]}" --header "$prompt" || return 1
        return 0
    fi
    ...
    while true; do
        ...
        printf "Invalid selection '%s'. Please enter a number between 1 and %d.\n\n" "$reply" "${#options[@]}" >&2
    done
}

show_usage() {
    cat <<'TEXT'
Usage: ghcli-setup [options]
...
show_welcome_message() {
    if have_cmd gum; then
        gum style \
            ...
            --margin "1 0" \
            --width 80 \
            "$(gum style --foreground 141 --bold 'Flox GitHub Setup')

This wizard will walk you through two steps:

  Step 1 — Store a GitHub personal access token for CLI access.
  Step 2 — Choose how Git connects to github.com (HTTPS, SSH, or skip).

Notes:
  - HTTPS Git should use a personal access token, not an account password.
  - SSH mode can register an SSH key with GitHub.
  - Press Ctrl+C or type 'exit' at any prompt to stop."
    else
        cat <<'TEXT'
Flox GitHub setup
...
show_token_instructions() {
    if have_cmd gum; then
        gum style \
            ...
            --margin "1 0" \
            --width 80 \
            "$(gum style --foreground 141 --bold 'GitHub Personal Access Token')

You will need a token before continuing. To create one:

  1. Go to $(gum style --foreground 212 --underline 'https://github.com/settings/tokens')
  2. Click 'Generate new token' (classic)
  3. Select scopes: $(gum style --foreground 212 'repo'), $(gum style --foreground 212 'read:org') (minimum)

Typical uses:
  - GitHub CLI access: token with the API rights you need
  - Git HTTPS: repo scope for classic PATs, or matching access for fine-grained PATs
  - SSH key upload: token may also need admin:public_key scope"
    else
        cat <<'TEXT'
Create or choose a GitHub personal access token before continuing.
...
validate_github_token() {
    ...
    code="$(curl -s -o /dev/null -w '%{http_code}' -H "Authorization: Bearer $token" -H 'Accept: application/vnd.github+json' -H 'X-GitHub-Api-Version: 2022-11-28' https://api.github.com/user 2>/dev/null || true)"
    [[ "$code" == '200' ]]
}
...
validate_github_basic_auth() {
    ...
    code="$(curl -s -o /dev/null -w '%{http_code}' -u "$username:$password" -H 'Accept: application/vnd.github+json' -H 'X-GitHub-Api-Version: 2022-11-28' https://api.github.com/user 2>/dev/null || true)"
    [[ "$code" == '200' ]]
}
...
configure_git_ssh_transaction() {
    ...
    if [[ -z "$existing_key_id" ]]; then
        local upload_choice=''
        upload_choice="$(ui_select 'SSH key not found on your GitHub account' \
            'Upload key to GitHub now' \
            'Skip upload (key is already on GitHub)' || true)"
        case "$upload_choice" in
            'Upload key to GitHub now')
                SSH_UPLOADED_KEY_ID="$(upload_ssh_key "$token" "$ssh_key_path.pub" "$key_title" 2>/dev/null || true)"
                [[ -n "$SSH_UPLOADED_KEY_ID" ]] || {
                    warn "SSH key upload failed. Check that your token has admin:public_key scope."
                    [[ -n "$ssh_config_backup" && -e "$ssh_config_backup" ]] && rm -f "$ssh_config_backup" || true
                    restore_git_key_from_file credential.https://github.com.helper "$helper_backup"
                    rm -f "$helper_backup" >/dev/null 2>&1 || true
                    return 1
                ...
                ;;
            'Skip upload (key is already on GitHub)'|'')
                ;;
        esac
    fi

    set_gh_git_protocol ssh || {
        warn "Failed to set git protocol to SSH."
        rollback_git_ssh_transaction
        return 1
    }
    ...
    if ssh_smoke_test "$ssh_key_path"; then
        if [[ "$SSH_SMOKE_ROUTE" == 'ssh.github.com:443' ]]; then
            install_managed_github_ssh_config "$ssh_key_path" || {
                warn "Failed to install SSH config for ssh.github.com:443."
                rollback_git_ssh_transaction
                return 1
            }
        else
            clear_managed_github_ssh_config || {
                warn "Failed to clear managed SSH config."
                rollback_git_ssh_transaction
                return 1
            }
        fi
    ...
    warn "SSH smoke test failed. Could not connect to github.com via SSH with key '$ssh_key_path'."
    rollback_git_ssh_transaction
    return 1
}
...
setup_github_integration() {
    ...
    case "$selected_mode" in
        ...
        ssh)
            ...
            if configure_git_ssh_transaction "$github_token" "$ssh_key_path" "$ssh_title"; then
                git_storage='ssh key'
            else
